...
// Add these functions to your controllers.js file

// Add a new milestone to an order
controllers.addMilestone = async (req, res) => {
  try {
    const { title, description, date, dueDate } = req.body;
    const order = await Order.findById(req.params.orderId);
    ...
    if (req.user._id.toString() !== order.sellerId.toString()) {
      return res.status(403).json({
        error: true,
        message: 'Only the seller can add milestones'
      });
    }
    ...
    order.milestones.push(newMilestone);
    await order.save();

    const addedMilestone = order.milestones[order.milestones.length - 1];
...
// Update a milestone
controllers.updateMilestone = async (req, res) => {
  try {
    ...
    if (req.user._id.toString() !== order.sellerId.toString()) {
      return res.status(403).json({
        error: true,
        message: 'Only the seller can update milestones'
      });
    }
...
// Delete a milestone
controllers.deleteMilestone = async (req, res) => {
  try {
    const { milestoneId } = req.params;
    const order = await Order.findById(req.params.orderId);
    ...
    if (req.user._id.toString() !== order.sellerId.toString()) {
      return res.status(403).json({
        error: true,
        message: 'Only the seller can delete milestones'
      });
    }

    order.milestones.id(milestoneId).remove();
    await order.save();
...
controllers.approveMilestone = async (req, res) => {
  try {
    const { milestoneId } = req.params;
    ...
    if (req.user._id.toString() !== order.buyerId.toString()) {
      return res.status(403).json({
        error: true,
        message: 'Only the buyer can approve milestones'
      });
    }
...
// Request milestone revision (buyer action)
controllers.requestMilestoneRevision = async (req, res) => {
  try {
    const { milestoneId } = req.params;
...
controllers.completeMilestone = async (req, res) => {
  try {
    ...
    // Find milestone by ID instead of index
    const milestone = order.milestones.id(milestoneId);
    if (!milestone) {
      return res.status(404).json({ 
        error: true, 
